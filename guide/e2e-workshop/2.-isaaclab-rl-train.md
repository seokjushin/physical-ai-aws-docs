...
> ✅ **가장 안전한 방법** — 본인 인스턴스에서 다음 한 줄을 먼저 실행하고, 출력된 이름을 그대로 `docker run` 마지막 줄에 복사하세요. UserId 추측·스택 이름 매핑 모두 불필요합니다.
>
> ```bash
> docker images | grep isaaclab
> ```
>
> 예시 출력 (`UserId=seokjus`인 멤버):
>
> ```
> <NX1_ACCOUNT>.dkr.ecr.us-east-1.amazonaws.com/nx1/isaaclab-seokjus:latest   b6fdcade6b1a   25.4GB
> nx1/isaaclab-seokjus:latest                                                b6fdcade6b1a   25.4GB
> ```
>
> 두 줄 중 **짧은 쪽 (`nx1/isaaclab-<userId>:latest`)** 을 그대로 복사해 아래 `docker run` 마지막 줄에 붙입니다. 두 줄은 같은 이미지(IMAGE ID 동일)이지만 짧은 이름이 타이핑·복사 실수 적습니다.

NX1 환경에서는 UserData가 `nx1/isaaclab-<userId>:latest` 형식으로 빌드 + ECR 푸시까지 완료한 상태입니다.
...
위 `docker images` 출력의 이름을 그대로 복사 (또는 본인 `<userId>`로 치환):
...
>
> **⚠️ 스택 이름과 `UserId` 파라미터가 다를 수 있음**: 예를 들어 스택 이름이 `nx1-isaaclab-ilhwang`이라도 `UserId` 파라미터가 `ilhwang-c`로 배포된 경우 이미지는 `nx1/isaaclab-ilhwang-c:latest` 입니다. 항상 `docker images` 출력 또는 콘솔 → CloudFormation → 본인 스택 → **Parameters** 탭의 `UserId` 값을 기준으로 하세요 (또는 EC2 Name 태그 `nx1-isaaclab-<UserId>`).
