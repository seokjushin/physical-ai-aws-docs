...
> 🟦 **BEST NX1**: code-server는 CloudFront가 아니라 [0. NX1 셀프 배포 가이드](0.-nx1-self-deploy.md)의 **SSM 포트포워딩(8888)** 으로 접속합니다. 또한 워크샵 스크립트의 **Greengrass 런타임 자가프로비전**(`put-role-policy`·`iot:*`·`--provision true`)은 **NX1에서 차단**되므로, **강사가 CloudFormation으로 IoT thing/policy + 스코프드 TES role을 사전 프로비전**하고 `--provision false`로 설치합니다. 강사 안내를 따르세요.
