...
> 🟦 **BEST NX1**: ⚠️ AWS Batch 인프라(+CDK)는 **NX1 거버넌스 변환 범위 밖**입니다. RL Batch 학습은 Day1 **선택(교육용)** 이며, NX1 워크샵에선 **진행하지 않거나 강사 시연**으로 대체합니다.
...
```mermaid
sequenceDiagram
    participant User as DCV 인스턴스
    participant ECR as ECR
    participant Batch as AWS Batch
    participant EC2 as EC2 (GPU 노드)
    participant EFS as EFS

    rect rgb(240, 248, 255)
    Note over User,ECR: 준비 단계
    User->>User: Docker 이미지 빌드
    User->>ECR: docker push (이미지 업로드)
    end

    rect rgb(245, 245, 220)
    Note over User,EFS: 학습 실행
    User->>Batch: Job 제출
    Batch->>EC2: 인스턴스 자동 프로비저닝 (g6.12xlarge × 2)
    EC2->>ECR: 이미지 Pull
    EC2->>EC2: 멀티노드 분산 학습 (NCCL AllReduce)
    EC2->>EFS: 체크포인트 + TensorBoard 로그 저장
    end

    rect rgb(240, 255, 240)
    Note over User,EFS: 결과 확인
    User->>EFS: EFS 마운트로 결과 접근
    User->>User: 추론 / 시각화
    end
```
...
CDK 스택 배포 시 Batch Compute Environment에서 참조할 리소스들이 자동 생성됩니다. 아래 다이어그램은 CDK가 생성한 5개 리소스가 Compute Environment에 어떻게 조립되는지 보여줍니다.

```mermaid
flowchart LR
    subgraph CDK["CDK가 생성하는 리소스"]
        LT["Launch Template<br/>(GPU AMI + 250GB EBS)"]
        IP["Instance Profile<br/>(S3, ECS, EFS, SSM 권한)"]
        SG["Batch Security Group<br/>(자기참조 Inbound + All Outbound)"]
        SN["Private Subnet"]
        EFS["EFS File System"]
    end

    CE["<b>Compute Environment</b><br/>(3.8에서 생성)"]

    LT --> CE
    IP --> CE
    SG --> CE
    SN --> CE
    EFS -.->|"NFS 2049<br/>Batch SG 허용"| SG

    style CE fill:#ff9900,color:#fff
    style CDK fill:#f0f8ff,stroke:#147eba
```

AWS CloudFormation 콘솔에서 배포된 스택의 **Outputs** 탭을 열면 아래 값들을 확인할 수 있습니다.
...
<details>
<summary><mark style="color:orange;">ℹ️ [CLI 대안]</mark> 현재 할당량을 AWS CLI로 확인</summary>

```bash
# G/VT 인스턴스 vCPU 할당량 확인 (--region 필수)
...
3. 원하는 vCPU 수 입력 (예: 10명 × g6.4xlarge 16 vCPU = 160)
4. 요청 제출
...
{% hint style="info" %}
Number of nodes의 기본값이 1로 되어 있습니다. 기존 값을 선택하고 `2`로 덮어써야 합니다.
{% endhint %}
...
Amazon ECR 콘솔로 이동하여 `isaaclab-batch` 리포지토리의 URI를 복사합니다. 작업 정의로 돌아와서 Node ranges 아래의 Image에 복사한 URI를 붙여넣습니다.

<figure><img src="../../.gitbook/assets/image (28).png" alt=""><figcaption></figcaption></figure>

| 설정 | 값 | 이유 |
| --- | --- | --- |
| Image | isaaclab-batch ECR 이미지 URI | ECR 콘솔에서 복사한 URI를 붙여넣기. Isaac Sim + Isaac Lab + 학습 스크립트가 포함 |
...
**Command**는 Job이 실행될 때 컨테이너 내부에서 수행할 명령입니다. 멀티노드 분산학습 시, 학습된 모델(.pt 파일)과 TensorBoard 로그를 EFS에 공유 저장하려면 Command를 아래와 같이 설정합니다. skrl은 체크포인트와 함께 TensorBoard 이벤트 파일(`events.out.tfevents.*`)을 동일 디렉토리에 기록하므로, 이 설정만으로 학습 메트릭 모니터링과 추론 모두 DCV 인스턴스에서 바로 가능합니다.
...
**Additional configuration**을 확장하고, Environment variables에서 **Add environment variable**을 선택한 다음 아래 표의 값을 추가합니다.
...
{% hint style="warning" %}
환경 변수 값에 후행 공백(trailing space)이 포함되지 않도록 주의하세요. 공백이 들어가면 학습 스크립트가 파라미터를 올바르게 인식하지 못합니다.
{% endhint %}                                             |
...
**Next**를 선택한 다음 **Create job definition**을 선택하여 생성을 완료합니다.
...
Job Queue를 생성한 후에는 Queue에 Job을 제출할 수 있습니다. Isaac Lab 강화 학습을 위한 멀티노드 및 멀티 GPU Job을 생성하고, AWS Batch의 Job Queue에 제출합니다.

### 3.11.1 Job 제출

1. AWS Batch 콘솔의 왼쪽 탐색 창에서 **Jobs**를 선택합니다.
2. **Submit new job**을 선택합니다.
3. 아래 값을 입력합니다:
...
4. 나머지 값은 기본값으로 두고 **Submit**을 선택합니다.

<figure><img src="../../.gitbook/assets/image (30).png" alt=""><figcaption></figcaption></figure>

{% hint style="info" %}
Job Definition에 지정된 환경변수를 런타임에 재정의할 수 있습니다. 예를 들어 다른 태스크를 학습하려면 **Node overrides** 섹션에서 `TASK` 값을 변경하면 됩니다.
{% endhint %}

### 3.11.2 Job 상태 모니터링

Job을 제출하면 아래 상태를 순서대로 거칩니다:

| 상태 | 설명 | 예상 소요 시간 |
| --- | --- | --- |
| `SUBMITTED` | Job이 Queue에 등록됨 | 수 초 |
| `RUNNABLE` | 스케줄링 대기 중. Compute Environment가 EC2 인스턴스를 프로비저닝 | 3~10분 (GPU 인스턴스 가용성에 따라 변동) |
| `STARTING` | 인스턴스에 컨테이너 배포 중. ECR에서 이미지 Pull | 2~5분 (이미지 크기 ~50GB) |
| `RUNNING` | 학습 실행 중 | 약 12분 (100 iteration 기준) |
| `SUCCEEDED` | 정상 완료 | — |

{% hint style="warning" %}
Job이 `RUNNABLE` 상태에서 10분 이상 머물면 아래를 확인하세요:
- **vCPU 할당량 부족**: 3.7에서 확인한 Service Quota가 96 이상인지 재확인
- **인스턴스 가용성**: 해당 AZ에 g6.12xlarge 용량이 부족할 수 있음. 잠시 후 재시도
- **Compute Environment 상태**: CE가 `VALID` 상태인지 확인
{% endhint %}

### 3.11.3 실행 결과 확인

1. Job 상태가 **RUNNING**이 되면 해당 Job을 선택하고 **Nodes** 탭에서 2개의 EC2 노드가 할당된 것을 확인합니다.

<figure><img src="../../.gitbook/assets/image (33).png" alt=""><figcaption></figcaption></figure>

2. 각 노드의 **Log stream** 링크를 선택하면 CloudWatch Logs에서 학습 메트릭을 실시간으로 확인할 수 있습니다. Main Node(#0)의 로그에서 iteration 진행 상황과 reward 값을 볼 수 있습니다.

<figure><img src="../../.gitbook/assets/image (35) (1).png" alt=""><figcaption></figcaption></figure>

{% hint style="info" %}
Job이 시작되면 2개 노드의 8개 GPU에서 100 iteration을 완료하는 데 약 12분이 소요됩니다.
{% endhint %}
