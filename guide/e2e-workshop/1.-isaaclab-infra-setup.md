...
> 🟦 **BEST NX1**: 원본 가이드의 `CloudShell` · `cdk deploy` · `DcvUrl(public IP)` · `CloudFront(code-server)` 단계는 NX1에서 **모두 막혀 있습니다** (CloudShell 차단, CDK 미허용, public IP 미허용, CloudFront 미배포). 본 모듈은 **CloudFormation 콘솔 + SSM 포트포워딩**으로 진행하며, 자세한 접속 패턴은 [모듈 0 NX1 셀프 배포 가이드](0.-nx1-self-deploy.md) 참고. *원본 1.3·1.5(자원 설명·환경 확인)는 그대로 학습 가치가 있으니 본 모듈에서도 보존했습니다.*
...
원본의 `npx cdk deploy` 흐름은 NX1에서 다음으로 대체됩니다 (모듈 0의 Step 1과 동일 패턴).

> **이미 모듈 0을 따라 본인 스택을 만든 상태라면 1.2를 건너뛰고 1.3으로 가세요.** 본 절은 모듈 0이 없는 경우의 백업 안내입니다.

1. 콘솔 → **CloudFormation** → **Create stack** → *With new resources*
2. **Template source = Amazon S3 URL**:
   ```
   https://dibh-737138011740-us-east-1-cloudformation.s3.us-east-1.amazonaws.com/nx1/day1/day1-isaaclab.yaml
   ```
...
- 강사가 staging한 템플릿: `s3://dibh-737138011740-us-east-1-cloudformation/nx1/day1/`
