...
> - **(Day2 배포 후) 본인 Day2 스택의 S3 버킷 `nx1-groot-<userId>-<NX1_ACCOUNT>`** — `nx1-groot-*` 패턴으로 IAM 허용. checkpoint 파일도 여기 업로드 가능
> - **공용 staging 버킷 `<INSTRUCTOR_BUCKET>`** — 강사 staging 영역, 본인 prefix(`<userId>/checkpoints/...`) 정해서 사용
>
...
aws s3 cp /home/ubuntu/environment/efs/models/h1_rough/<timestamp>_ppo_torch/checkpoints/best_agent.pt \
  s3://nx1-groot-<userId>-<NX1_ACCOUNT>/checkpoints/h1_rough/best_agent.pt
...
aws s3 sync /home/ubuntu/environment/efs/models/h1_rough/<timestamp>_ppo_torch \
  s3://nx1-groot-<userId>-<NX1_ACCOUNT>/checkpoints/h1_rough/<timestamp>_ppo_torch/
```
...
```bash
# NX1
aws s3 cp s3://nx1-groot-<userId>-<NX1_ACCOUNT>/checkpoints/h1_rough/best_agent.pt \
