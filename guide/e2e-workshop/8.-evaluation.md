...
- [모듈 7](7.-vla-train-sagemaker.md) 완료 (fine-tuned 체크포인트 보유 — Policy Server를 띄울 수 있는 상태). NX1 워크샵 v1은 모듈 6(AWS Batch)을 사용하지 않습니다.
...
# Policy Server 실행 중인지 확인 (모듈 7에서 이미 시작했다면)
...
[모듈 7](7.-vla-train-sagemaker.md)(SageMaker)에서 fine-tuning한 checkpoint로 Policy Server를 띄웁니다. 모듈 8의 Policy Server는 **Day1 인스턴스 로컬 docker(ZMQ:5555)** 이며, 모듈 7의 SageMaker REST endpoint와는 별개입니다. 이미 실행 중이면 이 단계를 건너뛰세요.
...
# 모듈 7 SageMaker Training Job 산출물 경로: s3://${BUCKET}/output/<job-name>/output/model.tar.gz
# (run_pipeline.py output_path=s3://${BUCKET}/output). model.tar.gz는 단일 파일이므로 cp 사용.
aws s3 cp s3://${BUCKET}/output/<TRAINING_JOB_NAME>/output/model.tar.gz \
