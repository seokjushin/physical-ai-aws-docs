...
3. **Parameters** (`day2-user.yaml`의 실제 6개):

   | 키 | 값 | 메모 |
   |---|---|---|
   | `UserId` | 본인 ID (소문자/숫자/하이픈) | Day1과 동일 ID 권장. 리소스 prefix·user-profile 이름이 됩니다 |
   | `StudioDomainId` | `<STUDIO_DOMAIN_ID>` (default 그대로) | 강사가 채팅으로 확인. 기존 nx1 Studio 도메인 |
   | `SsoUserValue` | 본인의 IdC UserName *전체* (`<id>@doosan.com` 또는 `<id>@corp.doosan.com`), 또는 **비움** | local-part만 적으면 안 됩니다. AWS 팀은 `@doosan.com`, Bobcat 팀은 `@corp.doosan.com`. 헷갈리면 비우고 진행 → Studio 로그인 시 자동생성 |
...
2. **Applications 탭** 클릭 → **Amazon SageMaker Studio BEST NX1** 클릭 → 새 탭에서 Studio 열림
3. **본인이 day2-user 스택으로 만든 `<UserId>` user-profile을 명시 선택해서 진입** (중요)
   - Studio 콘솔 → **Domains → nx1 (`<STUDIO_DOMAIN_ID>`) → User profiles** 탭
   - 두 종류의 user-profile이 보일 수 있습니다:
     - **`<UserId>`** (예: `seokjus`) ← **이게 본인 day2-user 스택이 만든 NotebookRole 연결 프로파일. 이걸 클릭**
     - `<id>-doosan-com-XXX` 같은 자동생성 프로파일 (IdC 첫 진입 시 만들어짐) ← **사용 금지** — 도메인 default execution role을 쓰는데 KMS 권한이 없어 새 JupyterLab space 생성 시 `PermissionError: Unable to create EBS volume ... you don't have permissions to use the KMS key` 에러
   - 본인 `<UserId>` 클릭 → 우측 **Launch → Studio**
4. JupyterLab 새 private space 생성: 좌측 **Spaces** → **Create JupyterLab space** → 이름 `<userId>-private` → 인스턴스 `ml.t3.medium`(default 충분, 학습은 SageMaker Training Job으로 별도) → **Run**
5. 30~60초 후 JupyterLab 환경 진입

> **자동생성 user-profile (예 `seokjus-doosan-com-b7f`) 사용 금지**: IdC 첫 로그인 시 자동으로 만들어지는 이 프로파일은 도메인 default execution role(`db-best-nx1-core-sagemaker-DomainSpaceExecutionRole-...`)을 사용하는데 Studio EBS KMS 키(`a33a7909-...`) 권한이 없어 새 JupyterLab space 생성이 막힙니다. 본인 day2-user 스택이 만든 `<UserId>` 프로파일을 사용하면 NotebookRole에 KMS 권한이 박혀 있어 정상 동작합니다.

> **AWS account 탭의 `BESTNX1-Developer` / `SageMaker Studio NX1` 역할 클릭이 아닙니다** — IdC 포털에서 SageMaker Studio는 *Application*으로 등록되어 있어 Applications 탭에서 바로 진입합니다.
...
bucket = f"nx1-groot-{os.environ.get('USER','<userId>')}-<NX1_ACCOUNT>"  # 본인 userId로 변경
...
| Studio에서 `Create JupyterLab space` 시 `PermissionError: Unable to create EBS volume ... you don't have permissions to use the KMS key` | IdC 자동생성 user-profile(예 `<id>-doosan-com-XXX`)로 진입한 상태입니다. Studio 도메인 EBS는 NX1 KMS 키로 강제 암호화되는데 도메인 default execution role엔 그 KMS 권한이 없습니다. **§5.4.1 step 3대로 본인 `<UserId>` user-profile (NotebookRole 연결) 로 다시 진입**하세요. 본인 day2-user 스택을 아직 안 만들었다면 §5.2.2 먼저 |
