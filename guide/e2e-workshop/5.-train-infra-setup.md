...
> 🟦 **BEST NX1**: 공유 인프라(ECR `nx1/*`·CodeBuild·MLflow·Studio 도메인)는 **강사가 CloudFormation으로 사전배포**하며 참가자는 `cdk deploy`를 하지 않습니다(CloudShell 차단). 접속은 [0. NX1 셀프 배포 가이드](0.-nx1-self-deploy.md)의 SSM/콘솔. 학습 config의 **MLflow tracking server ARN은 강사 제공**.
