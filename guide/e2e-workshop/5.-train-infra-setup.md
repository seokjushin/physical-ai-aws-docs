...
## 5.4 학습 잡 launch 환경 — Day1 EC2 (Studio 노트북 우회)

> 🟦 **NX1 우회**: 원본 가이드는 SageMaker Studio JupyterLab 노트북에서 학습 잡을 launch하지만, NX1 Studio 도메인은 SSO-enabled + EBS KMS 강제라서 자동생성 user-profile이 새 JupyterLab space를 만들 때 `PermissionError: Unable to create EBS volume ... you don't have permissions to use the KMS key` 에러가 납니다 (도메인 default execution role의 KMS 권한 부족, Dimitri 영역). 본 워크샵은 **Day1 EC2 인스턴스에서 boto3로 학습 잡을 launch**하는 흐름으로 우회합니다 — Day1 Python 환경이 이미 boto3·sagemaker SDK를 갖고 있고, 학습 잡 자체는 PassRole로 day2-user 스택의 SageMakerRole을 받아 별개 컨테이너에서 실행되므로 Studio 없이도 정상 동작합니다.

### 5.4.1 Day1 EC2에서 boto3 동작 확인

본인 Day1 인스턴스(`nx1-isaaclab-<userId>`)에 SSM Session Manager 또는 code-server로 진입한 뒤:

```bash
sudo su - ubuntu
python3 -c "import boto3; print(boto3.__version__)"
# 1.x.x — Day1 Python 환경에 이미 설치되어 있음
```

### 5.4.2 본인 Day2 스택 Outputs 확인
...
resp = cf.describe_stacks(StackName="nx1-groot-<본인UserId>")
...
기대 출력:
```
BucketName = nx1-groot-<userId>-<NX1_ACCOUNT>
SageMakerRoleArn = arn:aws:iam::<NX1_ACCOUNT>:role/nx1-GR00TSageMakerRole-<userId>
NotebookRoleArn = arn:aws:iam::<NX1_ACCOUNT>:role/nx1-GR00TNotebookRole-<userId>
```

### 5.4.3 공유 MLflow + S3 sanity check
...
print("S3 bucket head:", s3.head_bucket(Bucket=bucket))
```

`MLflow: Created arn:aws:sagemaker:...` 와 S3 bucket HTTP 200이 보이면 모듈 7 진행 준비 완료입니다.

### 5.4.4 (선택) SageMaker Studio 진입은 *읽기 전용*으로만

Studio 콘솔에 진입할 수는 있지만 (Applications 탭 → Amazon SageMaker Studio BEST NX1) **새 JupyterLab space 생성은 KMS 권한 문제로 실패**합니다. Studio는 워크샵 v1에서 다음 용도로만 제한적으로 사용:
- 모듈 7 학습 잡이 끝난 후 **MLflow UI에서 metric/artifact 조회** (자동생성 user-profile 진입으로도 가능)
- **Model Registry 콘솔 view** — 등록된 모델 확인

학습 launch / 노트북 코드 실행은 모두 Day1 EC2에서 진행합니다.
