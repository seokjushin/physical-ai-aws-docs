...
본 워크샵 v1은 **GR00T-N1.6** base를 사용합니다. N1.7도 NX1에서 사용 *가능*하지만 워크샵 v1에선 일정상 제외했습니다 — 아래 박스 참고.

<details>
<summary><strong>왜 N1.7이 아니라 N1.6인가 (NX1 관점)</strong></summary>

**기술적으로는 NX1에서 N1.7도 동작합니다.** Cloud One egress probe(2026-05-25 / 2026-05-29)에서 `huggingface.co` + `cdn-lfs.huggingface.co`가 정상 도달·TLS 정상 통과 확인됐고, NX1은 SSM Parameter Store SecureString 또는 Secrets Manager로 토큰을 안전하게 주입하는 패턴이 이미 있습니다. 학습 컨테이너 `train.py`도 `SM_HP_HF_TOKEN=ssm:/groot/hf-token` 형식 이미 지원.

워크샵 v1에서 N1.6으로 간 이유는 **거버넌스 차단이 아니라 워크샵 시간 관리** 때문입니다:

1. N1.7 backbone인 [`nvidia/Cosmos-Reason2-2B`](https://huggingface.co/nvidia/Cosmos-Reason2-2B)가 **HuggingFace gated model** — 각 참가자가 본인 HF 계정으로 모델 페이지에서 라이선스 동의를 클릭해야 함
2. 9명이 각자 HF 계정 생성 → 라이선스 동의 → Access Token 발급 → 안전하게 강사·SSM에 전달 → 학습 잡 환경변수 주입 — 처음 해보는 멤버 기준 1인당 ~10~15분
3. 동의 안 한 멤버 1명이 학습 잡 실패하면 강사 디버깅 부담 + 다른 멤버 진행 차질

**N1.7로 진행하려면 추가로 필요한 단계** (워크샵 후 또는 다음 회차에 적용 가능):
- 각 참가자: HF 계정 → Cosmos-Reason2-2B 페이지 → "Agree and access repository" 클릭
- 각 참가자: HF Settings → Access Tokens → Read 토큰 발급
- 토큰을 NX1 SSM Parameter Store SecureString (또는 Secrets Manager)에 저장 (`/nx1-groot/<userId>/hf-token`)
- 학습 잡 hyperparameter에 `hf_token=ssm:/nx1-groot/<userId>/hf-token` 주입 (`train.py`가 SSM 자동 resolve)
- (사전) Dimitri에 "GR00T-N1.7 backbone weight 다운로드용 HF_TOKEN — Bedrock 정책의 *runtime LLM API* 와는 다른 카테고리(download credential)로 이해, 맞는지" 한 줄 확인

이 단계들을 워크샵 v2 또는 본 PoC fine-tune 작업에 통합할 계획입니다.

</details>

> **참고**: 본 PoC의 실제 모델은 GR00T 계열이 아니라 Physical Intelligence **π0.5** 입니다. 워크샵 v1은 GR00T-N1.6으로 *AWS / SageMaker / 인프라 흐름*을 익히는 것이 목적이고, π0.5 도입은 별도 spike 후 결정 메모를 통해 일정·범위 확정 예정.
