...
3. (필요 시) Studio 콘솔에서 **Domains → nx1 (`<STUDIO_DOMAIN_ID>`) → User profiles** 탭으로 본인 `<userId>` user-profile이 보이는지 확인
4. JupyterLab 또는 Code Editor 앱 시작 → 인스턴스 타입은 작은 것(`ml.t3.medium`)으로 시작, 학습은 SageMaker Training Job으로 별도 실행하므로 노트북 자체에 큰 GPU 불필요
