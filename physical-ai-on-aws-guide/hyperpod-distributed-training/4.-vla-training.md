...
## 4.9 Closed-loop 시뮬레이션 검증 (GR00T + Isaac Lab)

Open-loop(MSE)은 모델이 데이터를 잘 피팅했는지 확인하지만, 실제로 로봇이 작업을 완수하는지는 알 수 없습니다. 학습된 GR00T VLA 정책을 Isaac Sim 시뮬레이션에서 closed-loop으로 평가하면, 카메라 이미지 + 관절 상태를 받아 action을 출력하고 시뮬레이션에서 로봇을 직접 제어하는 전체 루프를 검증할 수 있습니다.
...
flowchart TB
    ...
    Head -- "srun · terminal 1" --> GROOT
    Head -- "srun · terminal 2" --> ISAAC

    subgraph Compute["Compute Nodes"]
        direction LR
        subgraph DevNode["dev partition"]
            GROOT["GR00T Policy Server<br/>port 5555"]
        end
        subgraph DebugNode["debug partition"]
            ISAAC["Isaac Sim<br/>Physics + Rendering"]
        end
        GROOT -- "action (EEF delta)" --> ISAAC
        ISAAC -- "obs (image + joint)" --> GROOT
    end

    FSx[("/fsx · Checkpoints · Containers")]
    Compute -. "shared mount" .-> FSx

    style Head fill:#fce4ec,stroke:#c62828
    style DevNode fill:#e8f5e9,stroke:#2e7d32
    style DebugNode fill:#e3f2fd,stroke:#1565c0
...
```bash
# GPU 노드 할당 (추론용)
srun --partition=dev --gres=gpu:1 --time=01:00:00 --pty bash
...
{% hint style="warning" %}
HyperPod에서 closed-loop을 실행하려면 두 개의 GPU가 필요합니다 (하나는 추론, 하나는 시뮬레이션). `ml.g6e.12xlarge` (4× L40S) 노드를 사용하세요.
{% endhint %}

{% hint style="info" %}
Isaac Lab 환경 설치가 필요합니다. [5. RL 학습 (Isaac Lab)](5.-rl-training.md)의 "4.5.1 Isaac Lab 환경 준비" 섹션을 먼저 완료하세요.

Isaac Lab on AWS EC2 인스턴스가 이미 있다면 더 간편하게 검증할 수 있습니다. [6. 시뮬레이션 검증](6.-simulation-verification.md)을 참조하세요.
{% endhint %}
