...
Warning: Permanently added '<IP>' (ECDSA) to the known_hosts file.
       ...
       __|  (_)
       _|   |    AWS Elastic Compute Cloud - Amazon Linux
...
```bash
# 레포지토리 클론 및 환경 설정 (컨테이너 이미지 다운로드 포함, ~10분)
...
이 스크립트가 수행하는 작업:
- GR00T N1.7 컨테이너 이미지 다운로드 (`gr00t-core:1.7.0`)
- Python 패키지 설치 (MLflow, HuggingFace Hub 등)
- 작업 디렉토리 생성 (`/fsx/datasets/groot`, `/fsx/checkpoints/vla`, `/fsx/scratch/logs`)
