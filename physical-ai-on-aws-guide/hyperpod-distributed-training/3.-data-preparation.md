---
description: S3에 학습 데이터를 업로드하고 FSx 자동 동기화 확인
---
...
| `episodes.jsonl` | 각 에피소드의 프레임 범위 및 메타 | `{"episode_index": 0, "start_frame": 0, "end_frame": 500}` |
| `tasks.jsonl` | 각 에피소드의 태스크 설명 | `{"task": "pick_cube", "language": "Pick the cube"}` |
...
# 영상 파일 확인
ls /fsx/datasets/groot/my-robot/videos/chunk-000/observation.images.top/
...
### 3.4 HuggingFace에서 GR00T 호환 데이터셋 직접 다운로드

Head Node에서 HuggingFace Hub의 공개 데이터셋을 직접 다운로드할 수도 있습니다. 아래는 실제 GR00T에서 사용 가능한 LeRobot v2 형식 데이터셋입니다:

```bash
# Head Node에서 실행 (setup_environment.sh 실행 후)
...
```bash
# 데이터셋 전체 크기 확인
du -sh /fsx/datasets/groot/aloha_sim_cube/
...
{% hint style="info" %}
다음 단계: 4.-vla-training.md에서 GR00T N1.7 Fine-tuning 작업을 SLURM으로 제출합니다.
{% endhint %}
