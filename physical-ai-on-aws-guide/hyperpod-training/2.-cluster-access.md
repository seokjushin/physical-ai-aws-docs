...
- **Compute 노드 자동 스케일링**: Compute 노드(`ml.g6e.12xlarge`, `ml.g6.12xlarge` 등)는 초기에 `instanceCount=0`으로 시작하며, 작업 제출 시에만 자동으로 프로비저닝됩니다. 학습이 끝나면 자동으로 종료되어 비용이 발생하지 않습니다.
...
CDK Output의 `JumpHostJumpKeyCommand` 값을 **그대로 복사**한 뒤, `> ~/.ssh/hyperpod-${USER_ID}-jump-key.pem`만 뒤에 붙여 실행하면 됩니다.
...
{% hint style="info" %}
아래 명령은 **로컬 PC 또는 CloudShell**에서 실행하세요. Jump Host 내부에서는 SageMaker API 권한이 없어 응답이 오지 않습니다.
{% endhint %}

**Step 1.** Head Node의 Instance ID를 확인합니다.

SageMaker 콘솔 → HyperPod clusters → `hyperpod-${USER_ID}` → **Instances** 탭에서 `head` 그룹의 Instance ID를 확인할 수 있습니다:

![HyperPod 콘솔에서 Instance ID 확인](../../.gitbook/assets/1778311112185.png)
...
**Step 2.** 확인한 Instance ID로 Private IP를 조회합니다:

```bash
aws sagemaker describe-cluster-node \
  --cluster-name hyperpod-${USER_ID} \
  --node-id <위에서 확인한 InstanceId> \
  --region us-east-1 \
