...
`MLFLOW_TRACKING_URI`에 들어갈 ARN은 배포된 CDK Output의 **MLflowMlflowTrackingArn**로 확인할 수 있습니다.
...
export MLFLOW_TRACKING_URI="arn:aws:sagemaker:us-east-1:<ACCOUNT_ID>:mlflow-tracking-server/hyperpod-${USER_ID}-mlflow"
...
{% hint style="info" %}
**MLflow URI 형식:**
- **Python SDK (학습 코드)**: ARN 형식 사용 — `arn:aws:sagemaker:us-east-1:<ACCOUNT_ID>:mlflow-tracking-server/hyperpod-${USER_ID}-mlflow`
- **브라우저 UI**: URL 형식 — `https://us-east-1.experiments.sagemaker.aws/mlflow/hyperpod-${USER_ID}-mlflow`
...
SageMaker Managed MLflow UI는 보안을 위해 **Presigned URL**을 통해서만 접속할 수 있습니다. CDK 출력의 `MLflowTrackingUri` URL을 브라우저에 직접 입력하면 **"Missing Tracking Server ARN"** 에러가 발생합니다.

{% hint style="warning" %}
**직접 URL 접속 불가:** `https://us-east-1.experiments.sagemaker.aws/mlflow/...` 형식의 URL은 브라우저에 직접 붙여넣어도 동작하지 않습니다. 반드시 아래 CLI 명령으로 Presigned URL을 생성해야 합니다.
{% endhint %}

AWS CLI로 임시 접속 URL을 생성합니다:

```bash
# Presigned URL 생성 (기본 만료: 5분)
aws sagemaker create-presigned-mlflow-tracking-server-url \
  --tracking-server-name hyperpod-${USER_ID}-mlflow \
  --region us-east-1
```

출력된 `AuthorizedUrl` 값을 브라우저에 붙여넣으면 MLflow UI에 접속됩니다.

### 만료 시간 설정

Presigned URL은 기본 5분 후 만료됩니다. `--expires-in-seconds` 옵션으로 최대 10시간(36000초)까지 연장할 수 있습니다:

```bash
# 10시간짜리 Presigned URL 생성
aws sagemaker create-presigned-mlflow-tracking-server-url \
  --tracking-server-name hyperpod-${USER_ID}-mlflow \
  --region us-east-1 \
  --expires-in-seconds 36000
```

{% hint style="info" %}
**왜 Presigned URL인가?**

MLflow 오픈소스 UI 자체에는 인증 레이어가 없습니다. AWS는 S3 Presigned URL과 동일한 방식으로, URL에 임시 인증 토큰을 포함시켜 접근을 제어합니다. URL이 만료되면 동일한 CLI 명령으로 새로 생성하면 됩니다.

| 용도 | 사용할 값 | 영구 여부 |
|------|-----------|-----------|
| Python SDK (`mlflow.set_tracking_uri()`) | ARN | O (IAM 인증) |
| 브라우저 UI 접속 | Presigned URL | X (임시, 최대 10시간) |
...
```bash
# MLflow 로깅 데모 실행
python3 /fsx/scratch/aws-physical-ai-recipes/hyperpod-training/mlflow/example_usage.py
```
...
| 증상 | 원인 | 해결 방법 |
|------|------|----------|
| "Missing Tracking Server ARN" | 브라우저에서 URL 직접 접속 | `aws sagemaker create-presigned-mlflow-tracking-server-url --tracking-server-name <name> --region <region>` 으로 Presigned URL 생성 후 접속 |
| "401 Unauthorized" | AWS 콘솔 미로그인 또는 Presigned URL 만료 | Presigned URL 재생성 후 접속 |
| "Connection refused" | MLflow 서버 다운 | 몇 분 대기 후 재시도 |
| 메트릭이 기록되지 않음 | MLflow URI 잘못됨 | `echo $MLFLOW_TRACKING_URI` 확인 (ARN 형식이어야 함) |
