...
{% hint style="info" %}
[모듈 5](5.-train-infra-setup.md)에서 `infra-groot-finetune` CDK 배포가 완료된 상태여야 합니다(선택사항). 본 모듈은 독립적으로 실행 가능하며, 모든 리소스를 AWS CDK(TypeScript)로 생성합니다. 아직 배포하지 않았다면 [모듈 5.2](5.-train-infra-setup.md#5.2-infra-groot-finetune-cdk-배포)을 참고하세요.
{% endhint %}
...
[AWS CDK](https://docs.aws.amazon.com/cdk/v2/guide/home.html) (TypeScript)로 스택을 생성하여 필요한 모든 리소스를 프로비저닝합니다. **CloudShell**에서 실행해주세요.

### 7.2.1 CDK 배포

같은 AWS 계정에 두 명 이상이 배포하거나 한 사람이 여러 환경(dev/staging)을 분리하려면 `-c alias=<id>`를 지정합니다. 모든 리소스 이름에 `-<alias>` postfix가 자동으로 붙고, 스택 이름은 `GrootSMTrainingJob-<alias>`가 됩니다.
...
```bash
cd ~/aws-physical-ai-recipes/training/groot-sagemaker/infra
npm install
```

IsaacLab 부모 스택 (`IsaacLab-Latest-${USER_ID}` or `IsaacLab-Stable-${USER_ID}`)에서 VPC/Subnet/AZ를 읽어 `cdk.context.json`에 캐시하기 위해 아래 명령어를 실행합니다. 캐시가 되면 이후 `cdk deploy`가 그 VPC/Subnet/AZ를 자동으로 사용합니다. 해당되는 부모 스택이 없으면 `cdk deploy`가 계정 default VPC를 자동 탐지합니다.

```bash
npx ts-node bin/resolve-parent-stack.ts ${USER_ID} us-east-1
```

**정상 출력**:

```
부모 스택 시도: IsaacLab-Stable-yoo (us-east-1)
부모 스택 발견: IsaacLab-Stable-yoo, PrivateSubnetId=subnet-007e...

해석된 파라미터:
  alias            : yoo
  region           : us-east-1
  vpcId            : vpc-0ca7...
  subnetIds        : subnet-007e...
  availabilityZone : us-east-1a
```

다음은 배포 명령어를 수행합니다. 스택 이름은 기본값 `GrootSMTrainingJob-<alias>`이 사용됩니다. 같은 계정에 두 사람이 alias 없이 배포하면 두 번째 배포가 `Resource ... already exists` 에러로 실패합니다. 워크숍/팀 환경에서는 항상 `-c alias=`를 지정하세요.

```bash
# 배포
npx cdk deploy \
    -c alias=${USER_ID} \
    -c bucketName=groot-sm-training-${USER_ID} \
    -c region=us-east-1 \
    --require-approval never > deploy.log 2>&1 &

# 배포가 완료된 후 아래 명령어를 수행하면 config.yaml 파일에 리소스 정보가 저장됩니다.
# npx ts-node bin/update-config.ts --alias ${USER_ID} --region us-east-1
```

**정상 출력**:

```
GrootSMTrainingJob-yoo: deploying...
✅ GrootSMTrainingJob-yoo

Outputs:
  BucketName               = groot-sm-training-yoo
  SageMakerRoleArn         = arn:aws:iam::123456789012:role/GR00TSageMakerRole-yoo
  NotebookRoleArn          = arn:aws:iam::123456789012:role/GR00TNotebookRole-yoo
  TrainingRepositoryUri    = 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-training-yoo
  InferenceRepositoryUri   = 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-inference-yoo
  StudioDomainId           = d-xxxxxxxxxxx
  StudioUserProfileName    = groot-yoo
  MlflowTrackingServerArn  = arn:aws:sagemaker:us-east-1:123456789012:mlflow-tracking-server/groot-mlflow-yoo
  DeployEndpointLambdaArn  = arn:aws:lambda:us-east-1:123456789012:function:groot-deploy-endpoint-yoo
```

(선택) 배포가 완료된 후 아래 명령어를 수행하면 config.yaml 파일에 리소스 정보가 저장됩니다.
...
배포가 끝나면 AWS 콘솔 → SageMaker AI → SageMaker Studio → `groot-n16-studio-${USER_ID}` 도메인에서 "Open Studio" 로 접속하면 학습 잡 / 파이프라인 / 모델 레지스트리 진행 상황을 시각적으로 확인할 수 있습니다. 
...
`bin/update-config.ts`는 실행한 머신의 `config.yaml`만 채웁니다. CloudShell에서 인프라를 배포한 뒤 EC2/로컬에서 빌드·학습·배포를 이어가려면 두 환경의 `config.yaml`을 동기화해야 합니다 (또는 작업 환경에서 `npx ts-node bin/update-config.ts --alias ${USER_ID}`를 다시 실행해도 됩니다).
...
가이드의 다음 단계들에서 `${USER_ID}`, `${BUCKET}`, `${ALIAS}`, `${REGION}`이 반복적으로 등장하므로 `~/.bashrc`에 한 번 등록해 두면 새 터미널 탭을 열 때마다 자동으로 로드됩니다.
...
echo "USER_ID=$USER_ID  BUCKET=$BUCKET  ALIAS=${ALIAS:-<none>}  REGION=$REGION"
```
...
CDK 스택이 SageMaker managed MLflow tracking server(`groot-mlflow[-{alias}]`)를 만들고, `bin/update-config.ts`가 ARN을 `config.yaml`의 `mlflow.tracking_server_arn`에 자동 기록합니다. 학습 스크립트는 이 ARN을 컨테이너 env(`MLFLOW_TRACKING_URI` / `MLFLOW_EXPERIMENT_NAME` / `HF_MLFLOW_LOG_ARTIFACTS=true`)로 자동 주입합니다.

**MLflow UI 접속 (presigned URL):**

```bash
aws sagemaker create-presigned-mlflow-tracking-server-url \
    --tracking-server-name "groot-mlflow-${ALIAS}" \
    --region "${REGION}" \
...
aws sagemaker stop-mlflow-tracking-server \
    --tracking-server-name "groot-mlflow-${ALIAS}" --region "${REGION}"
```
...
[SageMaker AI 콘솔 → Model registry](https://console.aws.amazon.com/sagemaker/home#/model-registry) → Model groups → `groot-n16-models${ALIAS:+-$ALIAS}` 에서 최신 버전이 `Approved` 상태인지 확인하면 됩니다. 별도 승인 절차가 필요하지 않습니다.
...
```bash
aws sagemaker stop-mlflow-tracking-server \
    --tracking-server-name "groot-mlflow${ALIAS:+-$ALIAS}" \
    --region "${REGION}"
```

다음 단계에서 CFN 스택을 삭제하면 MLflow 서버도 함께 삭제되므로 별도 작업 불필요.
...
`-c alias=` 미지정 시 기본 스택(`GrootSMTrainingJob`)이 대상이 됩니다. 또는 그대로 CloudFormation API를 써도 됩니다:
...
| `Resource of type 'AWS::ECR::Repository' with identifier 'groot-n16-...' already exists` | 같은 계정에 이미 누군가 alias 없이 배포함 | `-c alias=<yourid>`를 지정하여 다시 배포 |
...
| Pipeline DeployEndpoint Step 실패 (`AccessDenied`) | Lambda IAM 권한 부족 또는 SageMakerRole의 lambda:InvokeFunction 누락 | `cd infra && npx cdk deploy ...` 재실행으로 IAM 갱신, CloudWatch `/aws/lambda/groot-deploy-endpoint${ALIAS:+-$ALIAS}` 로그 확인 |
