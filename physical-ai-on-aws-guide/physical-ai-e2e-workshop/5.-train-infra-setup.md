...
```bash
npx cdk deploy GrootBatchTrainShared --require-approval never > shared-deploy.log 2>&1 &
# npx cdk deploy GrootBatchTrainShared -c grootVersion=n1.7  # N1.7로 배포할 경우
```

#### 사용자별 스택

`userId`만 지정하면 나머지 파라미터는 부모 스택에서 자동 조회됩니다:
...
```bash
# 2. 사용자 스택 배포
npx cdk deploy GrootBatchTrain-<userId> --require-approval never > deploy.log 2>&1 &
```
...
GrootBatchTrainShared.EcrRepositoryUri = 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-batch-train
...
```bash
CDK_DEFAULT_REGION=us-east-1 npx cdk deploy GrootBatchTrain-<userId> \
  -c vpcId=<VpcId> \
...
```mermaid
flowchart TB
    subgraph SharedStack["GrootBatchTrainShared (관리자, 1회)"]
        direction TB
        CB["CodeBuild\n(Docker 자동 빌드)"]
        ECR["ECR Repository\n(groot-batch-train)"]
        CB -->|"push latest"| ECR
    ...
    subgraph UserStack["GrootBatchTrain-<userId> (사용자별)"]
        direction TB
        JQ["Job Queue"]
        CE["Compute Env\n(G6E 2xl~48xl)"]
        JD["Job Definition"]
    ...
    style UserStack fill:#dceefb,stroke:#64b5f6
