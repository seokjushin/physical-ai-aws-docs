...
[모듈 5.2](5.-train-infra-setup.md#5.2-infra-groot-finetune-cdk-배포)에서 배포한 `infra-groot-finetune` CDK의 CodeBuild가 빌드한 ECR 이미지를 그대로 사용합니다. 기본값은 **N1.6**이며, CDK 배포 시 `-c grootVersion=n1.7`을 지정했다면 N1.7 이미지가 사용됩니다.
...
{% hint style="info" %}
[모듈 5](5.-train-infra-setup.md)에서 `infra-groot-finetune` CDK 배포와 CodeBuild 이미지 빌드가 완료된 상태여야 합니다. 아직 배포하지 않았다면 [모듈 5.2](5.-train-infra-setup.md#5.2-infra-groot-finetune-cdk-배포)을 먼저 진행하세요.
{% endhint %}
...
npx cdk destroy GrootBatchTrain-<userId>
```
