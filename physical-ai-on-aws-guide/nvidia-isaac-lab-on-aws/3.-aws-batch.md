...
| Instance role | `BatchEC2ComputeResourceRole` | CDK가 생성한 IAM Role. ECR 이미지 풀, EFS 마운트, CloudWatch 로그 전송 권한 포함 |

### 3.6.1 Instance configuration
...
| Launch templates | `BatchEC2LaunchTemplate` | CDK가 생성한 템플릿. NVIDIA 드라이버 설치, EFS 자동 마운트, Docker 런타임 설정 등 GPU 학습에 필요한 초기화를 자동 수행 |
...
| 설정 | 값 | 이유 |
| --- | --- | --- |
| VPC ID | `IsaacLab-VPC` | CDK가 생성한 VPC. DCV 인스턴스, EFS와 동일 네트워크에 위치해야 EFS 접근 가능 |
| Subnets | `IsaacLab-PrivateB` | Private Subnet에 배치하여 인터넷에서 직접 접근 불가. NAT Gateway를 통해 ECR 이미지 풀 수행 |
| Security groups | (default) | 동일 VPC 내 통신 허용. 멀티노드 간 NCCL 통신(TCP)과 EFS NFS 통신에 필요 |

**Create compute environment** 를 선택하여 생성을 완료합니다.

***

## 3.7 Job Definition 생성
...
### 3.7.6 Mount points configuration
