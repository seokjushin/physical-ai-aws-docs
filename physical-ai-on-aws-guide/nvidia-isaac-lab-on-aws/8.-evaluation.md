...
| 구분 | [모듈 5](5.-gr00t-n1.md) (추론 테스트) | 모듈 8 (Closed-loop 평가) |
|------|---------------------|--------------------------|
...
[모듈 5](5.-gr00t-n1.md)에서 CodeBuild가 빌드한 GR00T 컨테이너 이미지(ECR)로 Policy Server를 실행하고, [모듈 6](6.-finetune-batch.md)(AWS Batch) 또는 [모듈 7](7.-finetune-sagemaker.md)(SageMaker)에서 fine-tuning한 checkpoint를 로드합니다. Isaac Lab 컨테이너는 NGC에서 직접 pull하며, LeIsaac 패키지와 씬 에셋은 `run-isaaclab.sh`가 자동으로 설치합니다.
...
- [모듈 5](5.-gr00t-n1.md), [6](6.-finetune-batch.md), 또는 [7](7.-finetune-sagemaker.md) 완료 (Policy Server를 띄울 수 있는 상태)
...
[모듈 6](6.-finetune-batch.md)(AWS Batch) 또는 [모듈 7](7.-finetune-sagemaker.md)(SageMaker)에서 fine-tuning한 checkpoint로 Policy Server를 띄웁니다. 이미 실행 중이면 이 단계를 건너뛰세요.

### 8.2.1 Checkpoint 준비

Policy Server를 실행하려면 fine-tuned checkpoint가 필요합니다. [모듈 6](6.-finetune-batch.md)(VLA Fine-tuning on AWS Batch)을 완료했다면 EFS에, [모듈 7](7.-finetune-sagemaker.md)(VLA Fine-tuning on SageMaker)을 완료했다면 S3에 저장되어 있습니다:
