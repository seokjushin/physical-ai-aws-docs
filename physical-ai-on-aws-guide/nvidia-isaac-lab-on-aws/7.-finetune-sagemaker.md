...
ECR 리포지토리, IAM 역할, CodeBuild 프로젝트는 계정 단위 글로벌 네임스페이스를 사용합니다. 같은 계정에 두 사람이 alias 없이 배포하면 두 번째 배포가 `Resource ... already exists` 에러로 실패합니다. 워크숍/팀 환경에서는 항상 `-c alias=`를 지정하세요.
...
### 옵션 1: SageMaker AI Notebook에서 작업하는 경우
...
### 옵션 2: 다른 환경(EC2/로컬)으로 작업을 이어가는 경우
...
7.3 의 컨테이너 빌드가 백그라운드에서 도는 동안 진행하면 됩니다. SO-101 로봇의 leisaac-pick-orange 데이터셋을 S3에 업로드합니다:
...
`LightwheelAI/leisaac-pick-orange`는 SO-101 로봇용 데이터셋이지만 GR00T 내장 embodiment가 아니므로 (`NEW_EMBODIMENT`로 학습), **modality 정의 파일을 데이터셋과 함께 S3에 올려야** 학습 컨테이너가 차원/키 매핑을 올바르게 인식합니다. 누락되면 `KeyError: 'new_embodiment'`로 학습이 실패합니다.
