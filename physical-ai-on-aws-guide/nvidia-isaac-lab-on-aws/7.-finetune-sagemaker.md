...
{% hint style="info" %}
[모듈 5](5.-gr00t-n1.md)에서 `infra-groot-finetune` CDK 배포가 완료된 상태여야 합니다(선택사항). 본 모듈은 독립적으로 실행 가능하며, 모든 리소스를 CloudFormation으로 생성합니다. 아직 배포하지 않았다면 [모듈 5.2](5.-gr00t-n1.md#5.2-infra-groot-finetune-cdk-배포)을 참고하세요.
{% endhint %}
...
CloudFormation 스택이 SageMaker managed MLflow tracking server(`groot-mlflow[-{alias}]`)를 만들고, ARN을 `config.yaml`의 `mlflow.tracking_server_arn`에 자동 기록합니다. 학습 스크립트는 이 ARN을 컨테이너 env(`MLFLOW_TRACKING_URI` / `MLFLOW_EXPERIMENT_NAME` / `HF_MLFLOW_LOG_ARTIFACTS=true`)로 자동 주입합니다.
