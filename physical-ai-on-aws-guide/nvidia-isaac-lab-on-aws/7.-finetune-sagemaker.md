...
[AWS CDK](https://docs.aws.amazon.com/cdk/v2/guide/home.html) (TypeScript)로 스택을 생성하여 필요한 모든 리소스를 프로비저닝합니다. CloudShell에서 실행해주세요.
...
# IsaacLab 부모 스택 (IsaacLab-Latest-${USER_ID} or IsaacLab-Stable-${USER_ID})에서
# VPC/Subnet/AZ를 읽어 cdk.context.json에 캐시
...
스택 이름은 기본값 `GrootSMTrainingJob-<alias>`이 사용됩니다. 같은 계정에 두 사람이 alias 없이 배포하면 두 번째 배포가 `Resource ... already exists` 에러로 실패합니다. 워크숍/팀 환경에서는 항상 `-c alias=`를 지정하세요.

`IsaacLab-Latest-${USER_ID}` 또는 `IsaacLab-Stable-${USER_ID}` 스택의 PrivateSubnetId를 읽어 `cdk.context.json`에 캐시하면, 이후 `cdk deploy`가 그 VPC/Subnet/AZ를 자동으로 사용합니다. 해당되는 부모 스택이 없으면 `cdk deploy`가 계정 default VPC를 자동 탐지합니다.

**정상 출력**:

`resolve-parent-stack.ts`:
```
부모 스택 시도: IsaacLab-Latest-yoo (us-east-1)
...
배포가 끝나면 AWS 콘솔 → SageMaker → Studio → `groot-n16-studio-${USER_ID}` 도메인에서 "Open Studio" 로 접속하면 학습 잡 / 파이프라인 / 모델 레지스트리 진행 상황을 시각적으로 확인할 수 있습니다. 
...
이후 가이드의 모든 `python ...` 명령은 venv가 활성화된 셸에서 실행한다고 가정합니다. 최초 1회 다음을 수행해 venv를 만든 뒤, **`~/.bashrc`에 자동 활성화 라인을 추가**해 새 터미널을 열 때마다 자동으로 진입되도록 합니다:
...
이 절 이후의 명령어는 **JupyterLab Terminal**(File → New → Terminal)에서 실행합니다. 가이드의 다음 단계들에서 `${USER_ID}`, `${BUCKET}`, `${ALIAS}`, `${REGION}`이 반복적으로 등장하므로 `~/.bashrc`에 한 번 등록해 두면 새 터미널 탭을 열 때마다 자동으로 로드됩니다.

배포한 리소스 정보를 `config.yaml`에 동기화하고, 그 값을 셸 변수로 추출합니다:
