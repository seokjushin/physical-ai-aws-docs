...
============================================================
  ...
  MLflow 서버   : arn:aws:sagemaker:us-east-1:123456789012:mlflow-tracking-server/groot-mlflow-yoo
============================================================
config.yaml 업데이트 완료
```

> **MLflow tracking server 비용 알림**: SageMaker managed MLflow 서버(Small)는 always-on이며 약 **$0.64/hr** (약 $462/월) 과금됩니다. 학습 후 사용하지 않을 때는 `aws sagemaker stop-mlflow-tracking-server`로 반드시 정지하세요 (자세한 절차는 7.7 참고).
...
### CloudWatch metric — Training Job *Performance* 탭

`scripts/run_training.py` / `pipeline/run_pipeline.py`는 Estimator에 `metric_definitions`을 자동 주입합니다. HF Trainer가 stdout으로 출력하는 dict 로그(`{'loss': ..., 'grad_norm': ..., 'learning_rate': ...}`)를 정규식으로 파싱하여 CloudWatch에 발행:
...
- SageMaker Console → Training Jobs → 해당 job → *Performance* 차트
- CloudWatch Metrics → `/aws/sagemaker/TrainingJobs` namespace
- 또는 `aws sagemaker describe-training-job ... --query FinalMetricDataList`로 마지막 값 조회

> **첫 metric은 모델 로드 + dataset 캐싱(약 8–10분) 후 첫 step부터 발행됩니다.** 그 전엔 *Performance* 탭이 비어 있어도 정상이에요. `SecondaryStatus`가 `Training`으로 바뀌고 stdout에 `{'loss': ...}`가 찍히는 시점부터 차트가 그려집니다.

### MLflow tracking — run/metric/param/artifact 풍부 추적

CloudFormation 스택이 SageMaker managed MLflow tracking server(`groot-mlflow[-{alias}]`, Small ≈ $0.64/hr always-on)를 만들고, `infra/deploy_stack.py`가 ARN을 `config.yaml`의 `mlflow.tracking_server_arn`에 자동 기록합니다. 학습 스크립트는 이 ARN을 컨테이너 env(`MLFLOW_TRACKING_URI` / `MLFLOW_EXPERIMENT_NAME` / `HF_MLFLOW_LOG_ARTIFACTS=true`)로 자동 주입합니다 — 별도 인수 불필요.

> **GR00T 특이사항 — sitecustomize.py 패치 필요**
>
> GR00T `experiment.py`가 `report_to="wandb" if use_wandb else "none"`로 하드코딩하여, env만 주입해도 HF Trainer가 `MLflowCallback`을 자동 등록하지 않습니다. 이를 우회하려고 `container/training/sitecustomize.py`가 `transformers.TrainingArguments.__post_init__`을 monkey-patch하여 `MLFLOW_TRACKING_URI`가 설정되어 있으면 `report_to`에 `mlflow`를 강제 추가합니다.
>
> Script Mode 채널로 자동 주입되므로 **Docker 재빌드 없이도 적용**되지만, 이 파일이 `container/training/` 아래에 있어야 합니다. 학습 후 MLflow UI에 run이 보이지 않으면 가장 먼저 이 파일이 존재하는지 확인하세요.
...
`RegisterModel` Step은 학습 결과를 [Model Registry](https://docs.aws.amazon.com/sagemaker/latest/dg/model-registry.html)에 **`Approved` 상태로 자동 등록**합니다 (워크숍/데모 시나리오에서 학습 → 배포까지 한 흐름으로 보여주기 위함).
...
> 프로덕션 환경에서 사람 승인 게이트가 필요하다면 `pipeline/run_pipeline.py` 의 `approval_status="Approved"` 를 `"PendingManualApproval"` 로 바꾸고, 이 단계에서 `aws sagemaker update-model-package --model-approval-status Approved` 로 명시 승인하세요.
...
LambdaStep이 실패했거나, S3 URI를 직접 지정해 ad-hoc으로 배포하고 싶으면 `scripts/deploy_endpoint.py`를 사용합니다:

```bash
# Model Registry의 최신 Approved 모델로 재배포
python scripts/deploy_endpoint.py

# 특정 model.tar.gz 직접 배포
python scripts/deploy_endpoint.py \
...
| Pipeline DeployEndpoint Step 실패 (`AccessDenied`) | Lambda IAM 권한 부족 또는 SageMakerRole의 lambda:InvokeFunction 누락 | `infra/deploy_stack.py` 재실행으로 IAM 갱신, CloudWatch `/aws/lambda/groot-deploy-endpoint${ALIAS:+-$ALIAS}` 로그 확인 |
