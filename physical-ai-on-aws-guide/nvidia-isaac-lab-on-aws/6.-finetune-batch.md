...
이 모듈에서는 AWS Batch를 사용하여 [NVIDIA GR00T N1](https://developer.nvidia.com/gr00t) VLA(Vision-Language-Action) 모델을 fine-tuning합니다. [모듈 5](5.-gr00t-n1.md)에서 사전학습된 GR00T 모델로 추론을 테스트했다면, 이번 모듈에서는 **커스텀 로봇 데이터셋**에 맞게 모델을 미세조정합니다.

[모듈 5.2](5.-gr00t-n1.md#5.2-infra-groot-finetune-cdk-배포)에서 배포한 `infra-groot-finetune` CDK의 CodeBuild가 빌드한 ECR 이미지를 그대로 사용합니다. 기본값은 **N1.6**이며, CDK 배포 시 `-c grootVersion=n1.7`을 지정했다면 N1.7 이미지가 사용됩니다.
...
{% hint style="info" %}
[모듈 5](5.-gr00t-n1.md)에서 `infra-groot-finetune` CDK 배포와 CodeBuild 이미지 빌드가 완료된 상태여야 합니다. 아직 배포하지 않았다면 [모듈 5.2](5.-gr00t-n1.md#5.2-infra-groot-finetune-cdk-배포)을 먼저 진행하세요.
{% endhint %}
...
단일 GPU로 기본 동작을 확인했다면, 여러 노드와 GPU를 활용하여 학습 속도를 높일 수 있습니다. CDK는 단일 노드용(`GrootFinetuneJob`)과 멀티 노드용(`GrootFinetuneMultiNodeJob`) 두 개의 Job Definition을 생성합니다.
...
```bash
aws batch submit-job \
  --job-name groot-finetune-distributed \
  --job-queue GrootFinetune-<userId>-GrootFinetuneQueue \
  --job-definition GrootFinetune-<userId>-GrootFinetuneMultiNodeJob \
  --region us-east-1
```
...
{% hint style="info" %}
**Job Definition 구분:**
- `GrootFinetuneJob` — 단일 노드용. `--container-overrides`로 환경변수 override
- `GrootFinetuneMultiNodeJob` — 멀티 노드용. `--node-overrides`로 환경변수 override. `targetNodes: "0:1"`은 모든 노드(0번과 1번)에 동일한 override를 적용합니다
{% endhint %}
