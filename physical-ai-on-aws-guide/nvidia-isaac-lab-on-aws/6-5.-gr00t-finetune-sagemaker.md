...
# AWS 자격증명 설정 (또는 환경변수: AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_DEFAULT_REGION)
aws configure
aws sts get-caller-identity   # 권한 확인
```
...
가상환경을 활성화하지 않고 명령마다 호출하려면 `python`을 `uv run python`으로 바꿔도 됩니다.
{% endhint %}
...
{% hint style="warning" %}
**빌드 실패 시:** CloudWatch 로그에서 `flash-attn` 또는 `CUDA` 관련 에러를 확인하세요. 재시도하면 보통 해결됩니다 — 동일 명령을 다시 실행하면 CodeBuild가 새 빌드를 큐에 넣습니다:
```bash
...
```json
{
    ...
    "ModelArtifacts": {
        "S3ModelArtifacts": "s3://<your-bucket>/output/groot-n16-training-yoo-2025-05-12-120000/output/model.tar.gz"
    },
...
데이터가 충분하고 시간이 있으면 전체 학습을 실행합니다 (multi-GPU로 더 빠르게):
...
| `--max-steps` | 100 | 총 학습 스텝 수 (config.yaml `training.max_steps`) |
| `--save-steps` | 50 | 체크포인트 저장 간격 |
| `--instance-type` | ml.g5.2xlarge | SageMaker 인스턴스 타입 (단일 GPU 검증용) |
| `--num-gpus` | 1 | 사용할 GPU 수 (instance-type에 따라 제한) |
| `--global-batch-size` | 32 | 글로벌 배치 크기 |
| `--groot-version` | n1.6 | GR00T 버전 (n1.6 또는 n1.7) |
| `--use-spot` / `--no-spot` | spot=on | Spot Instance 사용 여부 (config.yaml `training.use_spot`) |
...
가장 최근에 제출된 GR00T Training Job을 SageMaker API로 조회합니다 (이름 prefix는 ECR 학습 이미지 이름을 따라가므로 alias가 자동 반영됩니다):

```bash
NAME_PREFIX="groot-n16-training${ALIAS:+-$ALIAS}"
...
```bash
aws logs tail "/aws/sagemaker/groot-n16${ALIAS:+-$ALIAS}" \
...
Model saved to: s3://<your-bucket>/output/groot-n16-training-<alias>-<timestamp>/output/model.tar.gz
```
...
> `pipeline/run_pipeline.py`는 `--save-steps`를 받지 않습니다 (Pipeline은 `max-steps`, `global-batch-size`, `num-gpus`만 노출). 체크포인트 간격은 컨테이너의 학습 스크립트 기본값을 따릅니다.
...
| 배치 크기 | 32 | 32 |
| 학습률 | 1e-4 | 1e-4 |
| GPU | 1× g6e.2xlarge | 1× ml.g5.2xlarge (기본) |
...
# SageMaker 결과 (로컬에서)
aws s3 sync "s3://${BUCKET}/output/<job-name>/" ./sm-results/
