...
| [Amazon SageMaker Model Registry](https://docs.aws.amazon.com/sagemaker/latest/dg/model-registry.html) | 학습된 모델 버전을 저장하고 승인/관리하는 카탈로그 (공장의 품질 검사 통과 인장) |
| [Amazon SageMaker Endpoint](https://docs.aws.amazon.com/sagemaker/latest/dg/realtime-endpoints.html) | 배포된 모델을 REST API로 24/7 접근 가능하게 서빙하는 관리형 호스팅 (24시간 열려있는 추론 카운터) |
...
python infra/deploy_stack.py \
    --stack-name groot-n16-stack \
    --bucket-name my-org-groot-training-2025 \
    --region us-east-1
```
...
```
Creating CloudFormation stack: groot-n16-stack
Stack status: CREATE_IN_PROGRESS
Waiting for stack creation to complete... (약 3~5분)
Stack creation completed successfully
Stack ID: arn:aws:cloudformation:us-east-1:123456789012:stack/groot-n16-stack/xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx

Resources created:
  - S3 Bucket: my-org-groot-training-2025
  - IAM Role: groot-sagemaker-execution-role
  - ECR Repositories: groot-n16-training, groot-n16-inference
  - CodeBuild Projects: groot-n16-training-build, groot-n16-inference-build
  - CloudWatch Log Groups: /aws/sagemaker/groot-training, /aws/codebuild/groot-n16-training-build

Config file updated: config/stack_config.yaml
```

생성된 `config/stack_config.yaml`을 확인하여 리소스 정보를 메모합니다:

```bash
cat config/stack_config.yaml
```

출력 예:

```yaml
stack_name: groot-n16-stack
region: us-east-1
bucket_name: my-org-groot-training-2025
ecr_repository_training: 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-training
ecr_repository_inference: 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-inference
sagemaker_role_arn: arn:aws:iam::123456789012:role/groot-sagemaker-execution-role
```
...
Uploading to S3: s3://my-org-groot-training-2025/datasets/leisaac-pick-orange/
...
```
Triggering CodeBuild project: groot-n16-training-build
Build ID: groot-n16-training-build:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
Build status: IN_PROGRESS

Triggering CodeBuild project: groot-n16-inference-build
Build ID: groot-n16-inference-build:xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
Build status: IN_PROGRESS
...
ECR images pushed:
  Training: 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-training:latest
  Training: 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-training:n1.6
  Inference: 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-inference:latest
  Inference: 123456789012.dkr.ecr.us-east-1.amazonaws.com/groot-n16-inference:n1.6
```

빌드 진행 상황을 CloudWatch에서 확인합니다:
...
python scripts/run_training.py \
    --dataset-s3-uri s3://my-org-groot-training-2025/datasets/leisaac-pick-orange \
...
```json
{
    "TrainingJobName": "groot-n16-training-2025-05-12-120000",
    "TrainingJobArn": "arn:aws:sagemaker:us-east-1:123456789012:training-job/groot-n16-training-2025-05-12-120000",
    "ModelArtifacts": {
        "S3ModelArtifacts": "s3://my-org-groot-training-2025/output/groot-n16-training-2025-05-12-120000/output/model.tar.gz"
    },
...
```bash
aws logs tail /aws/sagemaker/groot-training \
    --region us-east-1 \
    --follow
```
...
Model saved to: s3://my-org-groot-training-2025/output/groot-n16-training-2025-05-12-120000/output/model.tar.gz
```
...
```
Creating SageMaker Pipeline: groot-n16-pipeline
Pipeline execution started: groot-n16-pipeline-execution-2025-05-12-120000
Execution status: Executing
Pipeline stages:
  1. Training → 2. Model Registry → 3. (선택적) Endpoint 배포
Waiting for pipeline to complete... (약 15분)
Pipeline execution completed
Model registered to Model Group: groot-n16-models
Model Package ARN: arn:aws:sagemaker:us-east-1:123456789012:model-package/groot-n16-models/1
```

<details>
<summary><strong>콘솔에서 수동 승인</strong></summary>

1. AWS Console 접속 → SageMaker → Model Groups → `groot-n16-models`
2. 최신 모델 패키지 선택 → "Update status" → "Approved" 선택
3. 또는 CLI:

```bash
MODEL_PACKAGE_ARN="arn:aws:sagemaker:us-east-1:123456789012:model-package/groot-n16-models/1"
...
Latest approved model: groot-n16-models (version 1)
Creating SageMaker Endpoint: groot-n16-endpoint
Endpoint creation in progress...
Waiting for endpoint to be InService (약 5-10분)

Endpoint Status:
  Name: groot-n16-endpoint
  Status: InService
...
메모: Endpoint 이름(`groot-n16-endpoint`)을 추론 테스트에 사용합니다.
...
```bash
python scripts/deploy_endpoint.py \
    --model-s3-uri s3://my-org-groot-training-2025/output/groot-n16-training-2025-05-12-120000/output/model.tar.gz \
...
# SageMaker 결과 (로컬에서)
aws s3 sync s3://my-org-groot-training-2025/output/<job-name>/ ./sm-results/
...
```
Deleting endpoint: groot-n16-endpoint
...
Stack groot-n16-stack deleted successfully
```

{% hint style="warning" %}
**ECR Repository:** Repository는 기본값으로 자동 삭제되지 않습니다. 수동 삭제가 필요하면:

```bash
aws ecr delete-repository \
    --repository-name groot-n16-training \
    --region us-east-1 --force

aws ecr delete-repository \
    --repository-name groot-n16-inference \
    --region us-east-1 --force
```
...
```bash
aws s3 rm s3://my-org-groot-training-2025 --recursive
```
