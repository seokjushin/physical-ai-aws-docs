...
========================================================
문제:
  Isaac Lab 2.x의 play.py가 최신 skrl API 기준으로 작성되어 있으나,
  Isaac Sim 5.1.0 컨테이너에 포함된 skrl 2.0.0과 호환되지 않음.
  - set_running_mode("eval") 메서드 없음
  - act() 시그니처 불일치: states + keyword-only timestep/timesteps 필요
패치 (원본 play.py에 한 번만 실행):
...
변경 내용:
  1. 탭 → 스페이스 4개로 통일 (TabError 방지)
  2. runner.agent.set_running_mode("eval") → pass (skrl 2.0.0에 메서드 없음)
  3. runner.agent.act(obs, timestep=0, timesteps=0) → runner.agent.act(obs, {}, timestep=0, timesteps=0)
     (skrl 2.0.0: positional arg 'states' 필수 + keyword-only timestep/timesteps)
     그 다음 다시 실행: 
...
만약 다음 버전의 조합으로 실행한다면
Isaac Lab 2.3.2 + Isaac Sim 5.1.0 조합의 알려진 호환 이슈(rsl_rl)로 인해 h1_locomotion.py 데모는 현재 환경에서 동작하지 않습니다.  
