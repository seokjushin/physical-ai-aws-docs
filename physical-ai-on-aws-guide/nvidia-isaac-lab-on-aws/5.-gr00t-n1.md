...
# 2. 배포 (기본값: N1.6)
npx cdk deploy --require-approval
...
모듈 4에서 사용했던 DCV 인스턴스에 접속하여, ECR에서 Pull한 이미지로 GR00T **base 모델**의 추론 서버를 실행합니다. 인스턴스에는 모듈 1에서 자동 구성된 `groot-inference.service`가 GPU와 5555 포트를 점유하고 있으므로, 새 컨테이너를 띄우기 전에 **먼저 기존 서비스를 중지**합니다.
