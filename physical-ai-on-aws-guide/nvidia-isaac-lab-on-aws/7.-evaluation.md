...
| 구분 | 모듈 5 (추론 테스트) | 모듈 7 (Closed-loop 평가) |
|------|---------------------|--------------------------|
...
모듈 5에서 CodeBuild가 빌드한 GR00T 컨테이너 이미지(ECR)로 Policy Server를 실행하고, 모듈 6에서 AWS Batch로 fine-tuning한 checkpoint(EFS)를 로드합니다. Isaac Lab 컨테이너는 NGC에서 직접 pull하며, LeIsaac 패키지와 씬 에셋은 `run-isaaclab.sh`가 자동으로 설치합니다.
...
- 모듈 5 또는 6 완료 (Policy Server를 띄울 수 있는 상태)
...
# Policy Server 실행 중인지 확인 (모듈 5/6에서 이미 시작했다면)
...
## 7.2 Policy Server 실행

모듈 6에서 fine-tuning한 checkpoint로 Policy Server를 띄웁니다. 이미 실행 중이면 이 단계를 건너뛰세요.

### Checkpoint 준비

Policy Server를 실행하려면 fine-tuned checkpoint가 필요합니다. 모듈 6(AWS Batch Fine-tuning)을 완료했다면 EFS에 이미 저장되어 있습니다:
...
huggingface-cli download hi-space/GR00T-N1.6-3B-Pick-Orange \
  --local-dir /home/ubuntu/environment/efs/GR00T-N1.6-3B-Pick-Orange
```

### Policy Server 컨테이너 실행
...
docker run -d --gpus all \
  ...
  -c 'cd /workspace/gr00t-repo && python3 -m gr00t.eval.run_gr00t_server \
    --model-path /mnt/efs/GR00T-N1.6-3B-Pick-Orange \
    --embodiment-tag NEW_EMBODIMENT \
    --host 0.0.0.0 \
    --port 5555'

# "Server ready" 확인 (약 30초~1분)
# PYTHONUNBUFFERED=1 덕분에 docker logs에 로그가 즉시 출력됩니다
docker logs -f groot-policy-server
```

---

## 7.3 LeIsaac 환경 구성 및 IsaacLab 컨테이너 실행
...
{% hint style="info" %}
스크립트 실행 결과로 **IsaacLab 컨테이너의 bash 쉘**에 진입합니다. 이후 7.4의 평가 명령은 이 컨테이너 내부에서 실행합니다.
{% endhint %}

---

## 7.4 Closed-loop 평가 실행
...
| `omni.appwindow` returns None | SSH에서 실행 (GUI 없음) | DCV 데스크탑에서 실행하거나, 7.3.5 headless 패치 적용 |
| LeIsaac import 실패 | 패키지 마운트 누락 | `-v ~/isaaclab-pkgs:/workspace/isaaclab-pkgs` 확인, `.leisaac-installed` 존재 확인 |
| 씬 로딩 실패 (USD not found) | 에셋 미다운로드 | 7.3.3 단계 재실행, `LEISAAC_ASSETS_ROOT=/assets` 환경변수 확인 |
